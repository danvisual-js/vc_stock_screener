...
const AI_PROXY_URL=""; // ← e.g. "/api/claude" once your proxy is deployed
...
// ── REAL-TIME QUOTES (Twelve Data) — credit-aware scheduler ────────
// Twelve Data's free tier allows only 8 credits/MINUTE and EACH symbol
// costs 1 credit (batching saves HTTP requests, not credits). A single
// load of a full tab + the 4 index tiles exceeds 8, so the API returns
// 429 and those symbols would otherwise silently keep their stale seed
// value — which is exactly what looked like "inaccurate prices", and why
// a freshly-added ticker showed $0. This scheduler paces requests to the
// budget and retries on 429, so every ticker eventually loads real data.
// For INSTANT full-tab loads, upgrade the Twelve Data plan or front this
// with the server proxy in quote-proxy.example.js (shared cross-user cache).
const QUOTE_TTL=30000;
const _quoteCache=new Map(); // SYM -> {t, q:{p,pc}}

const TD_LIMIT=8, TD_WINDOW=60000;
let _spends=[];                 // timestamps of recent credit spends
const _sleep=ms=>new Promise(r=>setTimeout(r,ms));
function _creditsLeft(){const now=Date.now();_spends=_spends.filter(t=>now-t<TD_WINDOW);return TD_LIMIT-_spends.length;}
function _spend(n){const now=Date.now();for(let i=0;i<n;i++)_spends.push(now);}
function _waitMs(){if(_spends.length<TD_LIMIT)return 0;return Math.max(0,TD_WINDOW-(Date.now()-_spends[0]))+300;}

// One HTTP request for a chunk of symbols. Throws "RATE" on a 429 so the
// scheduler can back off and retry. Returns {SYM:{p,pc}} for resolved ones.
async function _rawQuotes(syms){
  const r=await fetch(`https://api.twelvedata.com/quote?symbol=${encodeURIComponent(syms.join(","))}&apikey=${STOCK_API_KEY}`);
  const d=await r.json();
  if(d&&Number(d.code)===429)throw new Error("RATE");
  const rows=(syms.length===1)?{[syms[0]]:d}:d;
  const out={};
  for(const s of syms){
    const row=rows&&rows[s];
    const p=parseFloat(row&&row.close);
    if(p>0){const pc=parseFloat(row.previous_close);out[s]={p,pc:pc>0?pc:p};}
  }
  return out;
}

// Returns {SYM:{p,pc}} for every symbol that resolves (cache + live, paced
// to the credit budget). onPartial(delta) fires as each chunk lands so the
// UI fills in progressively instead of waiting for the whole tab.
async function fetchQuotes(symbols,onPartial){
  ...
  let guard=0;
  while(need.length&&guard<30){
    guard++;
    const left=_creditsLeft();
    if(left<=0){await _sleep(Math.min(_waitMs(),8000));continue;}
    const chunk=need.slice(0,left);
    _spend(chunk.length);
    try{
      const got=await _rawQuotes(chunk);
      const delta={};
      for(const s of chunk){if(got[s]){out[s]=got[s];delta[s]=got[s];_quoteCache.set(s,{t:Date.now(),q:got[s]});}}
      need=need.slice(chunk.length);
      if(onPartial&&Object.keys(delta).length)onPartial(delta);
    }catch(e){
      // 429 means our local counter drifted from the server's (other tabs/
      // users spent credits too) — assume the window is full and wait it out.
      _spends=Array(TD_LIMIT).fill(Date.now());
      await _sleep(Math.min(_waitMs(),8000));
    }
  }
  return out;
}
...
// UI still populates.
async function fetchPrices(symbols,onPartial){
  if(STOCK_API_KEY){
    const out=await fetchQuotes(symbols,onPartial);
...
export default function StockScreener(){
  ...
  const runRefresh=useCallback(async(stockList)=>{
    ...
    const merged=await fetchPrices(valid.map(s=>s.s),(delta)=>{
      // Progressive fill: apply each chunk as it lands so rows update live
      // instead of all-or-nothing after the whole tab resolves.
      setTabs(prev=>prev.map(t=>t.id===activeTab?{
        ...t,
        stocks:t.stocks.map(s=>{
          const u=delta[s.s]||delta[s.s?.toUpperCase()];
          if(!u)return s;
          const p=Number(u.p||0);
          const pc=Number(u.pc||p*0.99);
          return p>0?{...s,p,pc,loading:false,failed:false}:s;
        })
      }:t));
    ...
    if(Object.keys(merged).length>0){
      setTabs(prev=>prev.map(t=>t.id===activeTab?{
        ...t,
        stocks:t.stocks.map(s=>{
          const u=merged[s.s]||merged[s.s?.toLowerCase()]||merged[s.s?.toUpperCase()];
          if(!u)return s;
